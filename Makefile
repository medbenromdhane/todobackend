# Project variables
# the ?= means that the PROJECT_NAME variable is equal to 'some_value' if no value is there it'll take the default value 'todobackend'
...
# the BUILD_ID variable is used to get a unique execution each time it'll get the value from Jenkins BUILD_ID variable so ze avoid any conflicts with other apps containers build 
...
# .PHONY tell us that the code related to each name is not in a file but included into the Makefile file
